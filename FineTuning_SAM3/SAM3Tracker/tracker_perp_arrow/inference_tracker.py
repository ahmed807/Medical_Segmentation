...
def find_arrow_tip(annot_np: np.ndarray):
    ...
    # Step 1: Find the two points furthest apart (the arrow endpoints)
    ...
    # Step 2: Define the arrow's main axis
    axis      = end_b - end_a
    ...
    # Step 3: Get all annotation pixels
    ys, xs = np.where(annot_np > 127)
    if len(xs) == 0:
        return [int(end_a[0]), int(end_a[1])]
    mask_coords = np.column_stack([xs, ys]).astype(float)

    # Sample a band near each endpoint — a slice of the shape within
    # band_len of that endpoint along the axis. Measure how wide this
    ...
    def perpendicular_spread(endpoint):
        # Project each pixel onto the axis, measure distance along axis
        # from the endpoint. Keep only pixels within band_len.
        rel = mask_coords - endpoint
        along = rel @ axis_dir
        # At end_a, `along` is ~0 at the endpoint, positive toward end_b.
        # At end_b, `along` is ~0 at the endpoint, negative toward end_a.
        # We want pixels within band_len of each endpoint:
        mask = np.abs(along) < band_len
        if not mask.any():
            return 0.0
        # Measure perpendicular spread: max - min of perp coordinate
    ...
    # Step 4: Tip = endpoint with LARGER perpendicular spread (arrowhead flare)
