...
RUN_NAME = "v3_arrow_fix_freeform_mask"
...
def create_run_dir() -> dict:
    ...
    with open(info_path, "w") as f:
        ...
        f.write(f"|---|---|\n")
        f.write(f"| arrow | Point prompt at tip (two-endpoint pointy-end algorithm) |\n")
...
def find_arrow_tip(annot_np: np.ndarray, radius: int = 15):
    """
    Finds the arrowhead tip by identifying both endpoints of the arrow
    and selecting the POINTY end (fewer nearby pixels) over the WIDE end
    (tail/feathers have more nearby pixels).

    Algorithm:
      1. Find the two contour points furthest apart (the two ends).
      2. For each endpoint, count annotation pixels within a radius.
      3. The endpoint with FEWER nearby pixels is the tip (narrow/pointy).
      4. The endpoint with MORE nearby pixels is the tail (wide/feathered).
    ...
    # Step 3: Tip = the endpoint with fewer nearby pixels
