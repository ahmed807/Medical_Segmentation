...
FLUX_PROMPT         = "a clean photograph, natural background, no drawings, no markings, no annotations, no arrows, no boxes, no labels, no dashed lines, no dotted lines, seamless"
FLUX_GUIDANCE_SCALE = 30
FLUX_STEPS          = 50

# Mask preprocessing for Flux Fill.
# We close dashed-line gaps and add a small blending margin around the
# annotation. Dilation is ADAPTIVE — it scales with annotation size so
# tiny annotations in tight spaces don't accidentally expand into nearby
# objects, while large annotations still get enough blending margin.
FLUX_MASK_CLOSE_KERNEL = 7      # closes gaps in dashed lines
FLUX_MASK_DILATE_MIN   = 2      # minimum dilation in pixels (small annotations)
FLUX_MASK_DILATE_MAX   = 8      # maximum dilation in pixels (large annotations)
FLUX_MASK_DILATE_RATIO = 0.015  # dilation = diag * ratio, clamped to [min, max]
# ───────────────────────────────────────────────────────────────────────────────


# ── PROMPT HELPERS ─────────────────────────────────────────────────────────────

def find_arrow_tip(annot_np: np.ndarray, radius: int = 20):
    """
    Finds the arrowhead tip by identifying both endpoints of the arrow
    and selecting the WIDE end (arrowhead triangle) over the NARROW end
    (shaft tail).

    Key insight: the arrowhead is a triangle/V shape that flares OUT,
    so it has MORE pixels packed within a small radius than the tail
    (which is just the end of a thin shaft).

    Algorithm:
      1. Find the two contour points furthest apart (the two arrow ends).
      2. For each endpoint, count annotation pixels within `radius`.
      3. The endpoint with MORE nearby pixels is the tip (arrowhead spread).
      4. The endpoint with FEWER nearby pixels is the tail (thin shaft end).
    ...
    count_a = np.sum(np.linalg.norm(mask_coords - end_a, axis=1) < radius)
    count_b = np.sum(np.linalg.norm(mask_coords - end_b, axis=1) < radius)

    # Tip = the endpoint with MORE nearby pixels (arrowhead spread)
    tip = end_a if count_a >= count_b else end_b
...
def run_single(image_path: str,
    ...
    if sam_only or flux_pipe is None:
        ...
        # Preprocess the annotation mask: close dashed gaps + dilate for
        # blending margin. Critical for thin/dashed annotation lines —
        # without this, Flux struggles to fully erase them.
