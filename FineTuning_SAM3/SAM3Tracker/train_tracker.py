...
CHECKPOINT_EVERY = 6
BEST_MODEL_PATH  = "/home/ahma/Medical_Segmentation/FineTuning_SAM3/SAM3Tracker/sam3_best.pth"
LOG_DIR          = "/home/ahma/Medical_Segmentation/FineTuning_SAM3/SAM3Tracker/runs/sam3_finetune"
...
def combined_loss(pred, target):
    """BCE-with-logits + Dice. Both autocast-safe."""
...
def main():
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"SAM3 Tracker Fine-tuning — {NUM_EPOCHS} Epochs | Best val: {best_val_loss:.4f}")
    plt.legend()
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    plt.savefig("loss_curve_tracker.png", dpi=150)
    plt.close()

    print(f"\nDone.")
    print(f"  Best val loss : {best_val_loss:.4f}")
    print(f"  Best model    : {BEST_MODEL_PATH}")
    print(f"  Loss curve    : loss_curve_tracker.png")
