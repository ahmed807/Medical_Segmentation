...
def load_sam(model_path: str, device: str = DEVICE):
    ...
    if is_base:
        print(f"Loading ORIGINAL base Sam3TrackerModel from {SAM_MODEL_ID}...")
    else:
        print(f"Loading FINE-TUNED Sam3TrackerModel from {model_path}...")
...
def run_batch(dataset_dir: str,
    ...
    # Allow callers to point at a custom JSON (e.g. pipeline_b_input_default.json)
    # rather than the default test.json under dataset_dir. Pipeline B converter
    # emits absolute paths so dataset_dir is essentially a no-op for that JSON.
    ...
    for entry in tqdm(test_data, desc="Inference", dynamic_ncols=True):
        ...
        # GT segmap is only available in Pipeline A; Pipeline B detections have
        # no matching GT. Tolerate the absence — IoU computation is skipped
        # downstream when gt_segmap_path is None.
        gt_segmap_path  = (os.path.join(dataset_dir, entry["annotation"])
                           if entry.get("annotation") else None)

        # Pipeline B uses per-detection IDs that differ from the per-(image,
        # annotation) IDs of Pipeline A. Use entry-level fields when present so
        # multiple detections on the same source image don't collide on disk.
        if "source_image" in entry and "detection_idx" in entry:
            src_stem  = os.path.splitext(os.path.basename(entry["source_image"]))[0]
            entry_id  = f"{src_stem}_det{entry['detection_idx']}"
        else:
...
def parse_args():
    ...
    p.add_argument("--model",    required=True,
                   help="Path to fine-tuned Sam3Tracker weights (.pth), OR 'facebook/sam3' to use the original base model")
    ...
    p.add_argument("--test_json", default=None,
                   help="Custom test JSON path (e.g. pipeline_b_input_default.json). "
                        "If omitted, looks for 'test.json' inside --dataset.")
