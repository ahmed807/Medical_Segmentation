...
# ── CONFIGURATION ──────────────────────────────────────────────────────────────
DATASET_DIR   = "/home/ahma/Medical_Segmentation/FineTuning_SAM3/SAM3Tracker/sam_finetuning_dataset"
...
RUN_NAME = "v6_ablation_no_iou_loss"
...
def create_run_dir() -> dict:
    ...
    with open(info_path, "w") as f:
        ...
        f.write(f"|---|---|\n")
        f.write(f"| arrow | Point prompt at tip (PCA-axis algorithm, snapped to mask) |\n")
...
def find_arrow_tip(annot_np: np.ndarray):
    ...
    Algorithm:
      ...
      3. Snap each end to the nearest ACTUAL annotation pixel.
      4. Measure perpendicular spread at each end (outer 25% band).
...
def combined_loss(pred_masks: torch.Tensor, targets: torch.Tensor,
    ...
    """
    Ablation variant: SAM-paper-style combined loss WITHOUT the IoU-MSE term.
        20 * focal_loss + dice_loss

    - Focal: focuses on hard boundary pixels (alpha=0.25, gamma=2)
    - Dice : region overlap, handles class imbalance
    - IoU-MSE term DROPPED for this ablation. iou_pred is accepted but unused
      so the function signature stays compatible with forward_pass().
    """
    l_focal = focal_loss(pred_masks, targets)
    l_dice  = dice_loss(pred_masks, targets)

    # iou_pred kept in signature for compatibility but not used in this run.
    _ = iou_pred  # silence linters; avoids accidental removal

    return 20.0 * l_focal + l_dice
