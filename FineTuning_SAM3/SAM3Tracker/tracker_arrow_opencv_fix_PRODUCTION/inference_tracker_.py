...
FLUX_MASK_DILATE_BBOX  = 2    # bbox outlines get minimal dilation
# ───────────────────────────────────────────────────────────────────────────────
...
def prepare_flux_mask(annot_np: np.ndarray,
    ...
    Strategy: dilate ONLY the raw annotation line pixels by a fixed amount.
    Never morphologically close gaps — closing dashed outlines creates
    ...
    Args:
        dilate_px: pixels of outward expansion around the annotation stroke.
                   Small for bbox outlines (2px), larger for arrows (4px).
...
def flux_remove_annotation(pipe, image: Image.Image,
    ...
    mask_np = np.array(annot_mask).astype(np.float32) / 255.0
...
def run_single(image_path: str,
    ...
    if sam_only or flux_pipe is None:
        ...
        # Per-type Flux settings:
        #   arrow/number_letter: moderate guidance + more dilation (small marks)
        #   freeform/rect bbox:  very low guidance + minimal dilation
        #       (low guidance = Flux barely modifies pixels, just erases stroke)
        if annotation_type in ("freeform_bbox", "rect_bbox"):
            flux_mask   = prepare_flux_mask(annot_np, dilate_px=FLUX_MASK_DILATE_BBOX)
