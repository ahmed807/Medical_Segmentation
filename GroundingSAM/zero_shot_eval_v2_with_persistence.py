...
def persist_predictions_for_image(out_root, entry, preds, image_np):
    """
    Persist raw GDINO+SAM2 predictions to disk so they can be fed into
    a downstream Pipeline B (SAM3Tracker + FLUX inpainting) without
    re-running the detector.

    Writes one PNG mask per detection into <out>/pred_masks/<image_stem>_<idx>_sam.png
    Returns a per-image dict suitable for inclusion in predictions.json.
    """
    H, W = image_np.shape[:2]
    image_stem = Path(entry["image"]).stem
    masks_dir = out_root / "pred_masks"

    pred_entries = []
    for idx, p in enumerate(preds):
        # Save SAM 2 mask (the higher-quality of the two mask strategies — wins on
        # all classes per the prompt sweep). Color masks are not persisted by default
        # because color thresholding is consistently worse on matched IoU.
        mask_rel = f"pred_masks/{image_stem}_{idx}_sam.png"
        mask_path = out_root / mask_rel
        # Save as 0/255 PNG so downstream OpenCV/PIL loaders treat it as binary.
        cv2.imwrite(str(mask_path), (p["mask_sam"].astype(np.uint8) * 255))
...
def run_eval(args):
    ...
    if args.save_predictions:
        (out / "pred_masks").mkdir(exist_ok=True)
    ...
    all_records = []
    all_predictions = []  # NEW: per-image prediction index for Pipeline B
    n_overlays = 0
    ...
    for entry in tqdm(all_entries, desc="Eval"):
        ...
        # ── 2b. Persist raw predictions (boxes + SAM 2 masks) for Pipeline B ──
        # Greedy_match() is destructive in the sense that records do not retain
        # the box coordinates or mask arrays. Persist here, before that runs,
        # so a downstream pipeline can consume the detections without re-running.
        if args.save_predictions and len(preds) > 0:
            all_predictions.append(
                persist_predictions_for_image(out, entry, preds, image_np)
            )
        elif args.save_predictions:
            # Keep a record of empty-detection images too, so the converter
            # can iterate the full test set and skip with awareness.
            all_predictions.append({
                "image": entry["image"],
                "width": W,
                "height": H,
    ...
    print("\n" + "=" * 110)
    print(f"ZERO-SHOT V2  |  prompt: {args.prompt}")
    ...
    if args.save_predictions:
        ...
        n_dets = sum(len(p["predictions"]) for p in all_predictions)
        print(f"[predictions] wrote {len(all_predictions)} image entries "
              f"with {n_dets} total detections to {out}/predictions.json")
        print(f"[predictions] wrote {n_dets} mask PNGs to {out}/pred_masks/")
...
if __name__ == "__main__":
    ...
    p.add_argument("--test-json",     default="/home/ahma/Medical_Segmentation/GroundingSAM/gdino_finetuning_dataset/test.json")
    p.add_argument("--dataset-root",  default="/home/ahma/Medical_Segmentation/GroundingSAM/gdino_finetuning_dataset")
    ...
    p.add_argument("--prompt",        type=str, default=PROMPT,
                   help="Category prompt for GDINO. Class phrases must contain "
                        "the substrings 'arrow', 'dashed'/'outline'/'line', and "
                        "'letter'/'number' so label_to_class() can map back.")
    ...
    p.add_argument("--save-predictions", action=argparse.BooleanOptionalAction, default=True,
                   help="Persist per-detection SAM 2 masks + boxes to "
                        "predictions.json + pred_masks/. Required for Pipeline B.")
    args = p.parse_args()
    run_eval(args)
